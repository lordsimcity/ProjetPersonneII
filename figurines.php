...
$alpha="SELECT prixFigurine,nomFigurine,imageFigurine FROM FIGURINE ORDER BY nomFigurine";
$trialpha=$connection->query($alpha);

$croissant="SELECT * FROM FIGURINE ORDER BY prixFigurine ";
$filtrecroissant=$connection->query($croissant);

$decroissant="SELECT * FROM FIGURINE ORDER BY prixFigurine DESC ";
...
<div class="jeux">
<form method="post" action ="figurines.php">
<select name ='tri'>
...
if(isset($_POST['tri'])){
    ...
    if($tri==2){
        ...
        echo "<div>";
        echo"<img src='".$row['imageFigurine']."'/>";
        echo "<h1>".$row['nomFigurine']."</h1>";
        echo "<p class='prix'>".$row['prixFigurine']."€</p>";
        echo "</div>";
    }
